...
class SEEMDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None, target_queries=None, target_vlp=None, task='seg', extra={}):
        ...
        # QxNxC
        query_embed = self.query_embed.weight.unsqueeze(1).repeat(1, bs, 1) # [Q, B, C], Q=101, C=512
        ...
        if self.task_switch['grounding'] and grounding_extra_flag:
            ...
            grounding_tokens = extra['grounding_tokens'] # [S, B, C]
        ...
        output, query_embed = self.attention_data.cross_attn_variables() # For panoptic and text segment, same as before
        ...
        outputs["pred_primitives"] = results["predictions_primitive"]
        return outputs

    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, layer_id=-1):
        decoder_output = self.decoder_norm(output)
        decoder_output = decoder_output.transpose(0, 1) # [Q, B, d] -> [B, Q, d]
        class_embed = decoder_output @ self.class_embed # [B, Q, d] @ [d, d] -> [B, Q, d]
        outputs_class = self.lang_encoder.compute_similarity(class_embed) # [B, Q, C] C: num_classes
        ...
        results = {
            "attn_mask": attn_mask,
            "predictions_primitive": decoder_output,
